...
class dronepi:
    ...
    def __init__(self):
        ...
        self.__hsv_threshold_hue = [129.64882331422993, 180.0]
        self.__hsv_threshold_saturation = [142.1762589928058, 240.78237410071944]
        self.__hsv_threshold_value = [0.0, 255.0]

        self.hsv_threshold_output = None

        self.__cv_dilate_0_src = self.hsv_threshold_output
        ...
        self.__cv_erode_src = self.cv_dilate_0_output
        ...
        self.__cv_dilate_1_src = self.cv_erode_output
        self.__cv_dilate_1_kernel = None
        self.__cv_dilate_1_anchor = (-1, -1)
        self.__cv_dilate_1_iterations = 1.0
...
if __name__ == "__main__":
    ...
    time.sleep(3)
    print(cameraConfigs[0].name)

    img = numpy.zeros(shape = (240, 360, 3), dtype=numpy.uint8)
    video = CameraServer.getInstance().getVideo(camera = cameras[0], name = None) 
    # loop forever
    while True:

        time0, img = video.grabFrame(img)
        if time0 == 0:
            print(video.getError())
            continue
        vision.process(img)
        print(len(vision.find_contours_output), " contours found")
        time.sleep(2)
